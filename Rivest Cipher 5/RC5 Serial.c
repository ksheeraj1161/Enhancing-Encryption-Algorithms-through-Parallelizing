...
void RC5_ENCRYPT(WORD *pt, WORD *ct) /* 2 WORD input pt/output ct */ { WORD i,
A=pt[0]+S[0], B=pt[1]+S[1];//The plain text blocks A and b by adding S[0] and S[1]
...
for (A=B=i=j=k=0; k<3*t; k++,i=(i+1)%t,j=(j+1)%c) /*L is an array formed on the basis of user entered secret key.S and L are mixed with user secret key. 3*t > 3*c */
...
{
if(i==0){
pt1[0]="Hello";pt1[1]="Sagar";}
if(i==1){
pt1[0]="Hello";pt1[1]="Anushka";}
if(i==2){
pt1[0]="Hello";pt1[1]="Ksheeraj";}
for(j=0;j<b;j++)
